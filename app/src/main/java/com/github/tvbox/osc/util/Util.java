...
public class Util {
    ...
    public static Intent getChooser(Intent intent) {
        ...
        for (ResolveInfo resolveInfo : App.getInstance().getPackageManager().queryIntentActivities(intent, 0)) {
            String pkgName = resolveInfo.activityInfo.packageName;
            if (pkgName.equals(App.getInstance().getPackageName())) {
    ...
    public static boolean isTvBox() {
        ...
        if (Configuration.UI_MODE_TYPE_TELEVISION == ((UiModeManager) App.getInstance().getSystemService(Context.UI_MODE_SERVICE)).getCurrentModeType()) {
            return true;
        }
